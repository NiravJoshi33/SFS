...
const config = {
  ...
  scene: [BootScene, MenuScene, LobbyScene, GameScene, GameStoreScene],
  scale: {
...
function updateButtonDisplay(walletInfo: Wallet | null) {
  ...
  if (walletInfo && connectButton) {
    ...
    const shortenedAddress = `${userFriendlyAddress.substring(
      0,
      6
    )}...${userFriendlyAddress.substring(userFriendlyAddress.length - 4)}`;
  ...
  buyButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const destinationAddress =
        "0QBZSLwTlJQ87Eb8Ts0REVRo3Z7xZx09yEh0bgLqtvsIyC2d"; // Replace with the actual contract address
